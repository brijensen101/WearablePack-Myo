...
[assembly: InternalsVisibleTo("MyoTrainer")]
...
namespace Thalmic.Myo
{
    public class Hub : IDisposable
    {
        ...
        public Hub()
        {
            if (libmyo.init_hub(out _handle, IntPtr.Zero) != libmyo.Result.Success)
            {
        ...
        {
            libmyo.pair_any(_handle, count, IntPtr.Zero);
        }
        ...
        {
            libmyo.pair_adjacent(_handle, count, IntPtr.Zero);
        }

        public void PairByMacAddress(UInt64 macAddress)
        {
            libmyo.pair_by_mac_address(_handle, macAddress, IntPtr.Zero);
        }
